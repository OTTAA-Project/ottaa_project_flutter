...
class FirebaseDatabaseService {
  ...
  Future<int> getPicNumber() async {
    final User? auth = firebaseRed.currentUser;
    final refNew = databaseRef.child('${auth!.uid}/Usuarios/Avatar/urlFoto/');
      ...
      /*if (fileExists == true && fileExists != null) {
       ...
       /* if (pictoOrGrupo) {
          await _fileController.writePictoToFile(
              data: fileDataPicts.toString(), language: languageCode);
   ...
   /* final instance = await SharedPreferences.getInstance();
    ...
    if (resNew.exists && resNew.value != null) {
     ...
     /* List<String> fileDataPicts = [];
      ...
      print(fileDataPicts.toString());
      print(fileDataPicts.first.toString());
      if (pictoOrGrupo) {
        await _fileController.writePictoToFile(
  ...
  Future<void> saveUserPhotoUrl({required String photoUrl}) async {
    final User? auth = firebaseRed.currentUser;
    final ref = databaseRef.child('${auth!.uid}/Usuarios/Avatar/');
    await ref.update({
      'name': photoUrl,
    });
  }
  ...
  Future<String> fetchUserPhotoUrl() async {
    final User? auth = firebaseRed.currentUser;
    final refNew = databaseRef.child('${auth!.uid}/Usuarios/Avatar/');
    final resNew = await refNew.get();
    if (resNew.exists) {
